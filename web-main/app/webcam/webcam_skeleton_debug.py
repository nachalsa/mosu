...
def draw_keypoints_wholebody_on_frame(frame, keypoints, scores, bbox=None, threshold=2.0):
    """프레임에 wholebody 키포인트와 스켈레톤을 그리는 함수"""
    num_points = 133
    
    print(f"🔧 좌표 변환 시작 - bbox: {bbox}")
    print(f"원본 키포인트 타입: {type(keypoints)}, 길이: {len(keypoints)}")
    
    # bbox가 있으면 원본 이미지 좌표로 변환
    transformed_keypoints = keypoints.copy()  # 원본 보존
    if bbox is not None:
        x1, y1, x2, y2 = bbox
        crop_width = x2 - x1
        crop_height = y2 - y1
        
        print(f"크롭 영역: ({x1}, {y1}) to ({x2}, {y2}), 크기: {crop_width}x{crop_height}")
        
        # 크롭된 이미지에서 원본 이미지로 키포인트 좌표 변환
        transformed_keypoints = []
        for i, kp in enumerate(keypoints):
            if len(kp) >= 2:
                # 서버에서 받은 키포인트는 크롭된 이미지 내의 좌표
                crop_x, crop_y = kp[0], kp[1]
                # 원본 이미지 좌표로 변환
                orig_x = crop_x + x1
                orig_y = crop_y + y1
                ...
                if i < 3:
                    print(f"키포인트 {i}: ({crop_x}, {crop_y}) -> ({orig_x}, {orig_y})")
            else:
                transformed_keypoints.append(kp)
    else:
        print("⚠️ bbox 없음, 키포인트 그대로 사용")
    ...
    for idx in range(min(num_points, len(transformed_keypoints), len(scores))):
        ...
        if scores[idx] > threshold:
            x, y = transformed_keypoints[idx][:2]  # x, y만 사용
            # 프레임 경계 확인
...
class WebcamCapture:
    ...
    def force_enable_skeleton(self):
        """강제로 스켈레톤 표시 활성화"""
        self.show_skeleton = True
        print("🔧 강제로 스켈레톤 표시 활성화됨")
        return "스켈레톤 표시: 강제 활성화"
    
    def force_enable_realtime(self):
        """강제로 실시간 번역 활성화"""
        self.realtime_translate = True
        print("🔧 강제로 실시간 번역 활성화됨")
        return "실시간 번역: 강제 활성화"
        """현재 상태 확인"""
        status = {
            '실시간 번역': self.realtime_translate,
            '스켈레톤 표시': self.show_skeleton,
            '포즈 데이터 존재': self.last_pose_data is not None,
            '포즈 임계값': self.pose_threshold,
            'FPS 카운트': self.realtime_fps
        }
        
        if self.last_pose_data:
            status['마지막 포즈 데이터'] = {
                '키포인트 수': len(self.last_pose_data.get('keypoints', [])),
                '스코어 수': len(self.last_pose_data.get('scores', [])),
            ...
            if self.show_skeleton and self.last_pose_data:
                try:
                    ...
                    if keypoints and scores:
                        # 디버깅: 첫 번째 몇 개 키포인트 출력
                        print(f"첫 3개 키포인트: {keypoints[:3]}")
                        print(f"첫 3개 스코어: {scores[:3]}")
                        print(f"바운딩박스: {bbox}")
