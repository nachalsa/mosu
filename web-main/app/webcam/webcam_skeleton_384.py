...
class WebcamCapture:
    ...
    def process_frame_with_yolo(self, frame):
        """서버로 프레임 전송 및 포즈 추정"""
        self.realtime_fps += 1
        print(f"realtime_fps: {self.realtime_fps}")
        
        try:
            # 프레임을 직접 서버로 전송
            ret, buffer = cv2.imencode('.jpg', frame)
            if ret:
                files = {'image': (f'{self.realtime_fps}.jpg', buffer.tobytes(), 'image/jpeg')}
                resp = requests.post(
                ...
                if resp.status_code == 200:
                    ...
                    try:
                        ...
                        if keypoints and scores:
                            ...
                            print(f"✅ 포즈 데이터 업데이트: 키포인트 {len(keypoints)}개")
                        else:
                            print("❌ 키포인트 또는 스코어 데이터 없음")
                            
                    except Exception as e:
                        print(f"서버 응답 처리 오류: {e}")
                else:
